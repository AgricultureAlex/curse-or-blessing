 ...
 */
@component
export class RandomSliceQuestions
  extends BaseScriptComponent
  implements IQuestionsController
{
  @ui.label("Aim for 3-10 questions per round")
  @input("int", "5")
  private readonly questionsPerRound: number;
  ...
  @input
  @hint(
    "Prepend a line with an index of current question and a total number of questions",
  )
  ...
  public nextQuestion(): boolean {
    ...
    if (this._currentQuestion >= 0) {
      ...
      this.currentTextSetter =
        (this.currentTextSetter + 1) % this.doubleBuffer.length;
    }
    ...
    if (
      0 <= this._currentQuestionView &&
      this._currentQuestionView < this.questionsPerRound
    ) {
      ...
      textSetter.setText(
        this.numerate(this.questions[this._currentQuestion].question),
      );
      textSetter.fadeIn();
      return true;
    } else {
  ...
  /** Appends a reveal label (e.g. "Blessing") to the currently displayed question text.
   *  Called by AnswerReveal via Lifecycle → onAnswer.
   *  The next call to nextQuestion() will naturally replace the text with the new question. */
  public appendReveal(suffix: string): void {
    if (this._currentQuestion < 0 || !this.questions) return;
    const q = this.questions[this._currentQuestion];
    if (!q) return;
    const textSetter = this.doubleBuffer[this.currentTextSetter];
    if (textSetter) {
      textSetter.setText(this.numerate(q.question) + "\n" + suffix);
    }
  }
